...
public class MyLangParser implements Parser {
	...
	private ForEach parseForEach() throws ParserException {
		consume(FOREACH);
		final var variable = parseVariable();
		consume(IN);
		return new ForEach(variable, parseExp(), parseBlock());
	}
	...
	private Vector parseVector() throws ParserException {
		consume(OPEN_VECTOR);
		final var expression1 = parseExp();
		consume(STMT_SEP);
		final var expression2 = parseExp();
		consume(CLOSE_VECTOR);
		return new Vector(expression1, expression2);
	}
